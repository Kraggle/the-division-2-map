		...
		if ($stmt->num_rows == 1) {
			...
			if ($insert_stmt = $mysqli->prepare("INSERT INTO tokens (token, email) VALUES (?, ?)")) {
				...
				if (!$insert_stmt->execute()) {
					...
					$subject = "Password Reset for the-division-map";
					if ($_SERVER['HTTP_HOST'] == 'localhost') 
						$uri = 'localhost/DivisionMap';
					else 
						$uri = 'the-division-map.com';
				    $message = "<html>
						<head>
							<title>Password Reset for the-division-map</title>
						</head>
						<body>
				    ...
				    $headers .= 'From: The-Division-Map<admin@the-division-map.com>' . "\r\n";
